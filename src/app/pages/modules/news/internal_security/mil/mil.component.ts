...
export class MilComponent implements OnInit {
      ...
      .subscribe((data: any) => {
        ...
        sub_cats.forEach((x, i) => {
          if (!sub_cat) {
            ...
            console.log('mil api newslist ',this.api.newsList)
          } else {
  ...
  getMap(event=null,ctx="m2m") {
    ...
    var x=ctx=="inland"?"paf_and_pn":this.api.sub_cats_t_internal[this.cat]
    this.api.fetch_locations(resolved_cat,x,this.corner,this.map_date[ctx])
    .subscribe((data:any)=>{
