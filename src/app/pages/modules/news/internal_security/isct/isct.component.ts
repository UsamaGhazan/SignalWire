...
@Component({
  selector: "app-isct",
  templateUrl: "./isct.component.html",
  styleUrls: ["./isct.component.css"],
})
export class IsctComponent implements OnInit {
  cat = "is";
  ...
  loadmap = true;
  ngOnInit(): void {
    const button = this.api.theme[this.cat]["buttonBg"];
    console.log(button);
  ...
  ngAfterViewInit(): void {
    const button = this.api.theme[this.corner][this.cat]["buttonBg"];
    console.log(button);
    this.setRootVariable("--buttonBg", button);
  }

  setRootVariable(variable: string, value: string): void {
    console.log("setRootVariable ", variable, value);
  ...
  fetch_sub_news(
    ...
    if (!sub_cat || sub_cat === "rloCombined") {
      console.log("inside rloCombined");
      ...
      if (
        (sub_cat !== "rloCombined" && context == "ro_and_do") ||
        context == "all"
      ) {
        // emptying the selected lists
        sub_cats.forEach((x, i) => {
          this.api.newsList[x] = [];
          this.api.newsList["rloCombined"] = [];
          this.page_number[x] = 1;
          this.page_number["rloCombined"] = 1;
          this.page_end[x] = false;
          this.page_end["rloCombined"] = false;
    ...
    this.api
      ...
      .subscribe(
        (data: any) => {
          ...
          sub_cats.forEach((x, i) => {
            if (!sub_cat) {
              ...
              this.api.newsList["rloCombined"] =
                this.api.newsList.ro_and_do.concat(
                  this.api.newsList.law_and_order,
                  this.api.newsList.protest
                );
            } else if (sub_cat === "rloCombined") {
              const formattedData = this.formatData(data[x], x);
              this.api.newsList["rloCombined"].push(...formattedData);
            } else {
              data[x] &&
                this.api.newsList[x].push(...this.formatData(data[x], x));
            }
          });
          console.log("is & ct this.api.newsList", this.api.newsList);
        ...
        () => {
          ...
          // // Assuming this.api.newsList["rloCombined"] is your array
          // const newsList: NewsItem[] = ;
          // Sort the array by published_date
          this.api.newsList["rloCombined"].sort(
            (a: NewsItem, b: NewsItem): number => {
              // Convert published_date strings to Date objects
              const dateA = new Date(a.published_date.replace(",", ""));
              const dateB = new Date(b.published_date.replace(",", ""));
          ...
          console.log(this.api.newsList["rloCombined"]);
        }
      );
  }
  ...
  formatData(arr, sub_cat) {
    ...
    arr.forEach((element) => {
      ...
      let ranking = element._source.ranking;
  ...
  changeFilter(event) {
    console.log("changefilter event ", event);
  ...
  loadMore(context, sub_cat) {
    ...
    let url = this.api.sections_main.mil.indian_forces.includes(sub_cat)
      ? "fetch_sub_news_cat"
      : "fetch_sub_news";
    console.log("inside loadmore sub_cat ", sub_cat);
  ...
  fetch_twitter_data(context = "tweet", sub_cat = "", lazy_flag = false) {
    console.log("ftd context ", context);
    console.log("ftd subcat ", sub_cat);
    let sub_cats = [];
    ...
    this.api
      ...
      .subscribe(
        (data: any) => {
          ...
          sub_cats.forEach((x, i) => {
            if (!sub_cat) {
              ...
              this.api.tweetList["tweetsCombined"] =
                this.api.tweetList.activist.concat(
                  this.api.tweetList.disaster,
                  this.api.tweetList.law_and_order,
                  this.api.tweetList.other_is,
                  this.api.tweetList.protest,
                  this.api.tweetList.ro_and_do,
                  this.api.tweetList.tsm
                );
              console.log("this.api.tweetList[x] ", this.api.tweetList);
            } else {
              data[x] &&
        ...
        () => {
          console.log('tweetxx ',this.api.tweetList["tweetsCombined"]);
          console.log('tweetyy ',this.api.newsList);
          ...
          // const newsList: NewsItem[] = ;
          // Sort the array by published_date
          this.api.tweetList["tweetsCombined"].sort(
            (a: NewsItem, b: NewsItem): number => {
              // Convert published_date strings to Date objects
