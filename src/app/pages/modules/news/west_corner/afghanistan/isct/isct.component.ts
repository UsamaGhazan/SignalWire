...
export class IsctAfgComponent implements OnInit {
  cat = "diplomat";
  corner = "west_corner";
  Dates: any = {};
  keywords: any = {};
  ...
  twitter_internal_subcats = ["Pakistan Corner"];
  constructor(
    ...
    if (!this.data.currentWestTab) {
      this.corner = "west_corner";
    } else {
      this.corner = "west_corner_iran";
    }
  ...
  loadmap = true;
  ngOnInit(): void {
    this.Dates["Pakistan Corner"] = "latest";
    this.keywords["Pakistan Corner"];
    this.page_number["Pakistan Corner"] = 1;
    ...
    this.fetch_sub_news("Pakistan Corner", false);
    ...
    this.api
      ...
      .subscribe({
        next: (data: any) => {
          ...
          const x = "Pakistan Corner";

          const formatedData = this.formatData(data.data[x], x);
          if (!this.api.newsList[x] || filter == true) {
            console.log("ran filters or first try");
            this.api.newsList[x] = formatedData;
          } else {
            console.log("RAn");
            this.api.newsList[x].push(...formatedData);
          }
        ...
        complete: () => {
          this.loading["Pakistan Corner"] = false;
        },
      });
  }
  ...
  changeFilter(event) {
    ...
    this.fetch_sub_news(context, true);
  }
  ...
  loadMore(context, sub_cat) {
    ...
    let url = this.api.sections_main.mil.indian_forces.includes(sub_cat)
      ? "fetch_sub_news_cat"
      : "fetch_sub_news";
    this.fetch_sub_news(context, false, url, sub_cat);
  }
