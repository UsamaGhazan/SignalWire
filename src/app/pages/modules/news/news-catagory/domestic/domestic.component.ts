...
export class DomesticComponent implements OnInit, AfterViewInit {
  cat = "domestic";
  corner = "east_corner";
  Dates: any = {};
  keywords: any = {};
  ...
  constructor(
    ...
    this.all_cats.forEach((x) => {
      ...
      this.page_number["combineSepratists"] = 1;
      this.page_number["combineXhair"] = 1;
      this.page_number["tweetsCombined"] = 1;
      this.lazy_loader[x] = false;
      this.lazy_loader["tweetsCombined"] = false;
      this.lazy_loader["combineSepratists"] = false;
      this.lazy_loader["combineXhair"] = false;
      this.page_end[x] = false;
      this.page_end["combineSepratists"] = false;
      this.page_end["combineXhair"] = false;
  ...
  fetch_sub_news(
    ...
    if (
      !sub_cat ||
      sub_cat === "combineSepratists" ||
      sub_cat === "combineXhair"
    ) {
      ...
      this.Dates[context] = now
        ...
        : this.Dates[context];
      if (sub_cat !== "combineSepratists" && sub_cat !== "combineXhair") {
    ...
    console.log('domesticorner ',this.corner);
    this.api
      .fetch_sub_news(
        this.corner,
        sub_cats.join(","),
        ...
        this.corner==='east_corner' && this.cat==='domestic'?'domestic':''
      )
      .subscribe(
        (data: any) => {
          if (
          ...
          console.log("final subcatssss ", sub_cats);
          console.log("final subcat ", sub_cat);
          ...
          sub_cats.forEach((x, i) => {
            ...
            if (!sub_cat) {
              ...
              this.api.newsList["combineSepratists"] =
                this.api.newsList.naxal.concat(
                  this.api.newsList.nemove,
                  this.api.newsList.ladakh,
                  this.api.newsList.khalis
                );
              this.api.newsList["combineXhair"] = this.api.newsList.pm.concat(
                this.api.newsList.dm,
                this.api.newsList.ea,
                this.api.newsList.hm
              );       
            } else if (sub_cat === "combineSepratists") {
              const formattedData = this.formatData(data[x], x);
              this.api.newsList["combineSepratists"].push(...formattedData);
            } else if (sub_cat === "combineXhair") {
              const formattedData = this.formatData(data[x], x);
              this.api.newsList["combineXhair"].push(...formattedData);
            } else {
              console.log("else running");
              const formattedData = this.formatData(data[x], x);
              this.api.newsList[x].push(...formattedData);
            }
          });
          console.log("apinewslist domesticCombined ", this.api.newsList);
  ...
  loadMore(context, sub_cat) {
    console.log("loadmore subcat ", sub_cat);
          ...
          sub_cats.forEach((x, i) => {
            if (!sub_cat) {
              ...
              this.api.tweetList["tweetsCombined"] =
                this.api.tweetList.naxal.concat(
                  this.api.tweetList.nemove,
                  this.api.tweetList.ladakh,
                  this.api.tweetList.loksabha,
                  this.api.tweetList.polparties,
                  this.api.tweetList.pm,
                  this.api.tweetList.dm,
                  this.api.tweetList.ea,
                  this.api.tweetList.hm,
                  this.api.tweetList.khalis
                );
            } else {
              data[x] &&
