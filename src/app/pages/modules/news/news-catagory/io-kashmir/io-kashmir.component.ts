...
@Component({
  selector: 'app-io-kashmir',
  templateUrl: './io-kashmir.component.html',
  styleUrls: ['./io-kashmir.component.css']
})
export class IoKashmirComponent implements OnInit {
  cat = "iiojk"
  corner = "east_corner"
  Dates: any = {}
  keywords: any = {}
  ...
  constructor(private sanitizer: DomSanitizer,
    ...
    this.all_cats.forEach(x => {
      ...
      this.page_number['atinCombined']=1;
      this.page_number['odjCombined']=1
      this.page_number['iivCombined']=1
      this.lazy_loader[x] = false;
      this.lazy_loader['atinCombined']=false;
      this.lazy_loader['odjCombined']=false
      this.lazy_loader['iivCombined']=false
      this.page_end[x] = false;
      this.page_end['atinCombined']=false;
      this.page_end['odjCombined']=false
      this.page_end['iivCombined']=false
    });
...
//  loadmap=true;
  ...
  fetch_sub_news(context = 'all', url = 'fetch_sub_news', sub_cat = "", lazy_flag = false) {
    console.log('fetchsubnews context ',context)
    console.log('fetchsubnews subcat ',sub_cat)
      ...
      .subscribe((data: any) => {
        ...
        sub_cats.forEach((x, i) => {
          if (!sub_cat) {
            ...
            this.api.newsList['atinCombined']=this.api.newsList.atrocities.concat(this.api.newsList.indian_armedforces)
            this.api.newsList['odjCombined']=this.api.newsList.other_iiojk.concat(this.api.newsList.development_project,this.api.newsList.jihad_organiztion)
            this.api.newsList['iivCombined']=this.api.newsList.iijok_political_parties.concat(this.api.newsList.visits_jk)
          } else {
            data[x] && this.api.newsList[x].push(...this.formatData(data[x], x))
          }
        })
        console.log('this.api.newsList iokashmir',this.api.newsList)
        // this.api.newsList['atrocities'].push(...this.api.newsList['indian_armedforces'])
        // this.api.newsList['other_iiojk'].push(...this.api.newsList['development_project'],...this.api.newsList['jihad_organiztion'])
        // this.api.newsList['iijok_political_parties'].push(...this.api.newsList['visits_jk'])
  ...
  getMap(event=null,ctx="all") {
    let context, value = ""
    let resolved_cat = this.cat=='mil'?'army':this.cat=='intl'?'international':this.cat=='iiojk'?'kashmir':'domestic'
    if(event?.context) {
    ...
    this.api.fetch_locations(resolved_cat,this.api.sections_main[this.cat][ctx],this.corner,this.map_date[ctx])
    .subscribe((data:any)=>{
